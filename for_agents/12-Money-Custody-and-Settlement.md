# Croe — Money, Custody & Settlement (`12-Money-Custody-and-Settlement.md`)

> The backbone that makes escrow legal and correct. Uses the `CustodyProvider` interface from [`10-Architecture.md`](10-Architecture.md) §4 and the `transaction_ledger` sub-ledger from [`11-Data-Model.md`](11-Data-Model.md) §5. Custody phases P0–P3 per [`45-Glossary.md`](45-Glossary.md).
...
**Invariant (I1):** `pooled_account_balance(currency) == Σ held_balance(transaction) for that currency`, at all times, ignoring in-flight disbursements. Verified daily by reconciliation ([`42`](42-Observability-and-Reconciliation.md)).
...
A daily job (detailed in [`42-Observability-and-Reconciliation.md`](42-Observability-and-Reconciliation.md)) computes a `ReconciliationReport`:
...
**Any mismatch is a P1 incident** — freeze new disbursements for the affected currency and alert. Runbook in [`42`](42-Observability-and-Reconciliation.md).
